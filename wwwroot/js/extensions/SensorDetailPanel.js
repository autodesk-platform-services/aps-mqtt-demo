...
export class SensorDetailPanel extends Autodesk.Viewing.UI.DockingPanel {
    ...
    updateCharts(sensorId, dataView) {
        ...
        for (const [channelId, channel] of dataView.getChannels().entries()) {
            ...
            this._charts.push(this._createChart(canvas, (samples === null || samples === void 0 ? void 0 : samples.timestamps) || [], (samples === null || samples === void 0 ? void 0 : samples.values) || [], channel.min, channel.max, `${channel.name} (${channel.unit})`,channel.color));
        }
    }

    _createChart(canvas, timestamps, values, min, max, title,color) {
